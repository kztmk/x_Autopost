...
- `POST ?target=security&action=initialize`
  - 虎威側で Google Sheets URL と本人確認コードを保存したときに呼び出されます。
  - このエンドポイントのみ初回接続用として未認証で呼び出せます。
  - Spreadsheet メニューで生成した有効期限内の本人確認コードが必要です。
- `GET ?target=security&action=status`
  - 本人確認が完了しているかを確認します。

本人確認完了後、通常の `xauth` / `postData` / `postedData` / `errorData` / `trigger` API は、虎威の Firebase Functions proxy が付与する Firebase UID と HMAC 署名が必要です。
...
### 6.5 虎威連携セキュリティ (security.ts)

虎威から GAS Web App を安全に呼び出すための初期化と署名検証を提供します。

- `generateSetupCode()`: Spreadsheet メニューから本人確認コードを生成
- `initializeProxyAuth()`: 本人確認コードを検証し、Firebase UID と proxy secret を保存
- `getSecurityStatus()`: 本人確認の初期化状態を返却
- `assertProxyAuthorized()`: 通常 API リクエストの Firebase UID、timestamp、requestId、署名を検証
- `stripAuthField()`: POST ボディから署名用の `_auth` フィールドを除去

保存先は Apps Script の `PropertiesService` です。本人確認コードそのものや proxy secret はシートには書き込みません。

保存される主なプロパティ:

- `security_ownerUid`: この Spreadsheet を操作できる Firebase UID
- `security_proxySecret`: 虎威の Firebase Functions proxy と共有する署名用 secret
- `security_initializedAt`: 初期化完了日時
- `security_setupCodeHash`: 本人確認コードのハッシュ
- `security_setupCodeExpiresAt`: 本人確認コードの有効期限
...
1. 最新の `code.js` を Apps Script に反映します。
2. Apps Script エディタで保存します。
3. Web アプリを新しいバージョンとしてデプロイします。
   - `clasp push` やコード貼り付けだけでは、既存の `/exec` URL が古いバージョンを配信する場合があります。
   - Apps Script でコードを貼り付けた場合は、必ず保存ボタンを押してください。
4. 対象の Spreadsheet を再読み込みします。
5. Spreadsheet 上部メニューの `虎威連携` から `本人確認コードを生成` を選択します。
6. 表示された本人確認コードをコピーします。
   - 有効期限は 10 分です。
   - コード形式は `XXXXXXXX-XXXXXXXX-XXXXXXXX` です。
7. 虎威のプロフィール/API設定画面で以下を入力します。
   - GAS Web App の `/exec` URL
   - Spreadsheet メニューで生成した本人確認コード
8. 保存すると、虎威の Firebase Functions が `POST ?target=security&action=initialize` を呼び出します。
9. 初期化に成功すると、GAS 側には Firebase UID と proxy secret が保存され、虎威側には server-side secret が保存されます。
10. 初期化後、虎威側の GAS URL は表示専用になります。URLを変更する場合は変更ボタンを押し、Spreadsheet メニューから新しい本人確認コードを生成して再設定してください。

本人確認の初期化ログは Spreadsheet の `log` シートに記録されます。記録されるのは処理段階、action/target、マスク済み UID、本人確認コードの有無と文字数、エラーメッセージなどです。本人確認コードの値や proxy secret は記録されません。
...
5. レート制限に対応したリトライ機能
6. 虎威連携では Spreadsheet メニューで生成する本人確認コードを使って初回接続を行う
7. 初回接続後の通常 API は Firebase UID と HMAC 署名で検証する
8. requestId を CacheService に保存してリプレイ攻撃を防止する
9. timestamp の許容時間外のリクエストを拒否する
