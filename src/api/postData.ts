...
function createPostData(postDataInput) {
    ...
    // ヘッダー定義 (シートの列の順番)
    const headerColumns = ['id', 'createdAt', 'postTo', 'media', 'postSchedule', 'inReplytoInternal', 'postId', 'inReplyToOnX'];
...
function fetchPostData() {
    ...
    Logger.log(`Fetched ${postDataList.length} post data entries from sheet "${SHEET_NAME}".`);
    return postDataList;
  }
...
function updatePostData(postDataToUpdate) {
    ...
    for (let i = 1; i < values.length; i++) {
      ...
      if (row.length > idColumnIndex && row[idColumnIndex] === targetId) {
        ...
        if (createdAtColumnIndex !== undefined && row.length > createdAtColumnIndex) {
    ...
    // 対象IDが見つからなかった場合
    if (targetRowIndex === -1) {
      throw new Error(`PostData with ID "${targetId}" not found in sheet "${SHEET_NAME}".`);
    }
    ...
    try {
      ...
      const newRowValues = headers.map(header => {
        ...
        if (headerKey === 'createdAt') {
          ...
          if (originalValueIndex !== undefined && values[targetRowIndex].length > originalValueIndex) {
      ...
      sheet.getRange(targetSheetRow, 1, 1, newRowValues.length).setValues([newRowValues]);
...
function deletePostData(postDataToDelete) {
    ...
    if (targetId === "all") {
      ...
      // ヘッダー行しかない、または空のシートの場合は何もしない
      if (lastRow <= 1) {
        Logger.log(`No data rows to delete in sheet "${SHEET_NAME}".`);
        return {
          status: 'success',
          message: 'No data rows found to delete.',
          deletedCount: 0
        };
      }
  
      try {
        const numRowsToDelete = lastRow - 1; // 削除するデータ行の数 (ヘッダーを除く)
        // 2行目から最後のデータ行までを削除
        sheet.deleteRows(2, numRowsToDelete);
        Logger.log(`Deleted ${numRowsToDelete} data rows from sheet "${SHEET_NAME}".`);
        return {
          status: 'success',
          message: `Successfully deleted all (${numRowsToDelete}) data rows.`,
          deletedCount: numRowsToDelete
        };
      } catch (e: any) {
        Logger.log(`Error during bulk deletion of PostData: ${e}`);
        throw new Error(`Failed during bulk deletion of PostData: ${e.message}`);
      }
      ...
      if (sheetRowToDelete === -1) {
        throw new Error(`PostData with ID "${targetId}" not found in sheet "${SHEET_NAME}". Cannot delete.`);
      }
  
      // --- 削除実行 ---
      try {
        sheet.deleteRow(sheetRowToDelete);
        Logger.log(`PostData with ID "${targetId}" deleted successfully from row ${sheetRowToDelete}.`);
        return {
          status: 'success',
